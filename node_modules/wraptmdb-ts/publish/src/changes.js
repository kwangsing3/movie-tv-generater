...
class Change {
     ...
     ********************/
    async GetMovieChangeList(start_date, end_date, page) {
        const token = c_module.GetToken();
        const header = c_module.GetHeader();
        let targetURL = baseURL + c_module.Route.MOVIE + 'changes' + `?api_key=${token}`;
     ...
     ********************/
    async GetTVChangeList(start_date, end_date, page) {
        const token = c_module.GetToken();
        const header = c_module.GetHeader();
        let targetURL = baseURL + c_module.Route.TV + 'changes' + `?api_key=${token}`;
        if (start_date !== '' && start_date !== undefined)
            targetURL += `&start_date=${start_date}`;
        if (end_date !== '' && end_date !== undefined)
            targetURL += `&end_date=${end_date}`;
     ...
     ********************/
    async GetPersonChangesList(start_date, end_date, page) {
        const token = c_module.GetToken();
        const header = c_module.GetHeader();
        let targetURL = baseURL + 'person' + '/changes' + `?api_key=${token}`;
        if (start_date !== '' && start_date !== undefined)
            targetURL += `&start_date=${start_date}`;
        if (end_date !== '' && end_date !== undefined)
            targetURL += `&end_date=${end_date}`;
