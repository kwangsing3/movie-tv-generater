...
class Companies {
     ...
     ********************/
    async GetDetails(company_id) {
        const token = c_module.GetToken();
        const header = c_module.GetHeader();
        const targetURL = baseURL + c_module.Route.COMPANY + `${company_id}` + `?api_key=${token}`;
     ...
     * @doc https://developers.themoviedb.org/3/companies/get-company-details
     ********************/
    async GetAlternativeNames(company_id) {
        const token = c_module.GetToken();
        const header = c_module.GetHeader();
